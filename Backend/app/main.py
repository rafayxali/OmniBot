...
from .tts import transcribe_audio_with_groq
...
async def chat_voice(
    ...
    print(f"📥 Incoming VOICE request: session={session_id}, voice={voice_gender}")
    ...
    try:
        ...
        # 2. STT (GROQ - FIXED)
        # -----------------------------
        transcribed_question = transcribe_audio_with_groq(
            audio_bytes,
            file.filename
        )

        print(f"🗣️ Transcribed: {transcribed_question}")

        if not transcribed_question or not transcribed_question.strip():
        ...
        full_response_text = "".join(token_generator)

        print(f"🤖 Response ready: {full_response_text[:80]}...")

        # -----------------------------
        # 4. RETURN TO FRONTEND (NO TTS)
        # -----------------------------
        return JSONResponse(
            status_code=200,
            content={
                "transcribed_text": transcribed_question,
